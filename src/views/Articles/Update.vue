...
<button
      ...
      <div>
        <FormulateInput
          ...
          name="image"
        />
        <span v-if="this.imageSrc"><img :src="this.imageSrc"/></span>
        <span v-if="!this.imageSrc">No image selected</span>
        <span>
          <FormulateInput
            type="button"
            label="Choose image"
      ...
      validation="required"
    />
        <div><button @click="saveArticle" class="main">Save</button></div>
        <div><button @click="deleteArticle" class="delete">Delete</button></div>
      </div>
    </div>
...
import GET_ARTICLE from "@/queries/getArticle.gql";
import GET_TEAM from "@/queries/getTeam.gql";
import GET_PLAYER from "@/queries/getPlayer.gql";
import DELETE_ARTICLE from "@/queries/deleteArticle.gql";
import UPDATE_ARTICLE from "@/queries/updateArticle.gql";
...
export default {
  ...
  computed: {
    imageSrc: function() {
      if (this.article.image) {
        ...
        return `https://res.cloudinary.com/pley-gg/image/upload/w_200/${this.article.image.publicId}`
      }
      return null
    },
  ...
  async mounted() {
    ...
    const articleResponse = await this.$apollo.mutate({
      mutation: GET_ARTICLE,
      variables: {
        ...this.$props
      },
    }).catch((error) => {
    ...
    let article = articleResponse.data.getArticle
    article.publishedAt = this.timestampToFormDateTime(article.publishedAt)
    delete article.image.__typename
    delete article.__typename

    if (article.teamMentions) {
      article.teamIds = []
      for (let teamMention of article.teamMentions) {
        teamMention = `[${teamMention.id}] ${teamMention.name}`
        article.teamIds.push({ id: teamMention })
      }
    }

    if (article.playerMentions) {
      article.playerIds = []
      for (let playerMention of article.playerMentions) {
        playerMention = `[${playerMention.id}] ${playerMention.name}`
        article.playerIds.push({ id: playerMention })
      }
    }
    ...
    if (this.editor) this.editor.setContent(JSON.parse(article.body));
  ...
  methods: {
    ...
    async saveArticle() {
      ...
      let { playerIds, teamIds } = this.article
      let playerMentions = []
      if (playerIds) {
      ...
      if (playerMentions) articleData.playerMentions = playerMentions 
      delete articleData.playerIds

      let teamMentions = []
      if (teamIds) {
        /*for (let teamId of teamIds) {
          teamId = teamId.id
          const idPosEnd = teamId.indexOf(']')
          let teamMention = {
            id: parseInt(teamId.substr(1,idPosEnd-1)),
            name: teamId.substr(idPosEnd+2)
          }
      ...
      if (teamMentions) articleData.teamMentions = teamMentions
      delete articleData.teamIds

      articleData.publishedAt = this.dateToTimestamp(articleData.publishedAt)
      articleData.body = this.body
